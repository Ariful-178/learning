...
test("001_LiveOrder&R_CMS_ 001 | Check the functionality of Create New App is working properly and delete properly", async ({ page,ReservationPage, AppsPage }) => {
    ...
    await test.step('Navigate to apps directory', async() =>{ 
        await page.goto('https://dragonflyadmin-dev.loompavision.com/#/apps/792353d2-b992-4760-85a9-09570f1fc77c/0b6fcffd-e117-43dd-8403-28688baa5ff7/live-order',{waitUntil:"domcontentloaded"})
    })
    ...
    await test.step('Check the functionality of textbox and dropdown field', async() =>{ 
        await AppsPage.input_app_name_2('A_Test_app_name')
    })

    

    await test.step('input app slug', async() =>{ 
        await AppsPage.input_app_slug_2('Test_app_slug')
    })
    ...
    await test.step('Check the functionality of textbox and dropdown field', async() =>{ 
        await AppsPage.Click_Reservations()
    ...
    await test.step('Check the functionality of textbox and dropdown field', async() =>{ 
        await AppsPage.type_search_in_field('A_Test_app_name')
        await AppsPage.Click_action_btn_for_Test_app_name()

        
    })
    await test.step('Check the functionality of textbox and dropdown field', async() =>{ 
        await AppsPage.Click_remove_btn()

        
    })
    await test.step('Check the functionality of textbox and dropdown field', async() =>{ 
        await AppsPage.Click_yes_btn()
...
test.skip("001_LiveOrder&R_CMS_ 003 | Check the Pagination is working properly", async ({ loginPage, page, AppsPage}) => {
...
test("001_LiveOrder&R_CMS_ 008 | Validate the Edit Details under Action button is working properly", async ({  page,ReservationPage,AppsPage }) => {
    ...
    await test.step('check make hidden visiblity', async() =>{ 
        await AppsPage.click_edit_details()
    })
    await test.step('check hidden visiblity', async() =>{ 
        await ReservationPage.check_explore_all_events_visiblity()
    })
...
test("001_LiveOrder&R_CMS_009 | Check the mandatory textbox field", async ({  page,ReservationPage,AppsPage }) => {
    ...
    await test.step('Check the functionality of textbox and dropdown field', async() =>{ 
        await AppsPage.select_create_new_app()
    })
